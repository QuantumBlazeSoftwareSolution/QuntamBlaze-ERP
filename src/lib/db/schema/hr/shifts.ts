...
export const shiftTypeEnum = pgEnum('shift_type', [
  'Morning', 'Evening', 'Night', 'Rotational', 'Fixed'
]);
...
export const shiftSwaps = pgTable('shift_swaps', {
  id: uuid('id').defaultRandom().primaryKey(),
  requestingEmployeeId: varchar('requesting_employee_id').references(() => employees.id).notNull(),
  targetEmployeeId: varchar('target_employee_id').references(() => employees.id).notNull(),
  
  originalShiftAssignmentId: uuid('original_shift_assignment_id').references(() => shiftAssignments.id).notNull(),
  targetShiftAssignmentId: uuid('target_shift_assignment_id').references(() => shiftAssignments.id).notNull(),
