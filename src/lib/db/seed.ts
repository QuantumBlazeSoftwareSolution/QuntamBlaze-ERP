...
async function main() {
  ...
  try {
    ...
    await db.insert(schema.users).values(MOCK_TEAM.map((u: TeamMember) => ({
      id: u.id, name: u.name, email: u.email, avatar: u.avatar, roleId: "role-admin", status: u.status, lastActive: u.lastActive ? new Date(u.lastActive) : null,
    })));
    ...
    await db.insert(schema.employees).values(MOCK_EMPLOYEES.map((e: Employee) => ({
      id: e.id, name: e.name, email: e.email!, role: e.role, department: e.department, status: e.status, phone: e.phone || null, address: e.address || null, nic: e.nic || null, bankDetails: e.bankDetails || null, profileHealth: e.profileHealth || 0, joinDate: e.joinDate ? new Date(e.joinDate) : null,
    })));

    console.log("Inserting Projects...");
    await db.insert(schema.projects).values(MOCK_PROJECTS.map((p: Project) => {
      const details = PROJECT_DETAILS[p.id];
      return { id: p.id, name: p.name, clientId: p.clientId, startDate: new Date(p.startDate), deadline: new Date(p.deadline), progress: p.progress, budget: String(p.budget), status: p.status, description: details?.description || "" };
    }));
    ...
    await db.insert(schema.invoices).values(MOCK_INVOICES.map((inv: Invoice) => ({
      id: inv.id, projectId: inv.projectId, clientId: inv.clientId, issueDate: new Date(inv.issueDate), dueDate: new Date(inv.dueDate), amount: String(inv.amount), tax: String(inv.tax), status: inv.status, billingAddress: inv.billingAddress, lineItems: inv.lineItems,
    })));
