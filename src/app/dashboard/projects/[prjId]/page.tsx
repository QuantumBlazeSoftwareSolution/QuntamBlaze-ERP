...
const KanbanBoard = React.lazy(() => import('@/components/projects/KanbanBoard').then(m => ({ default: m.KanbanBoard })));
const GanttView = React.lazy(() => import('@/components/projects/GanttView').then(m => ({ default: m.GanttView })));
...
const getProjectDetail = (id: string): ProjectDetail => {
  ...
  return {
    ...prj,
    description: "Legacy infrastructure migration to cloud-native architecture. Involves deprecating on-premise clusters and establishing multi-region redundancy across three availability zones.",
    ...
    linkedDocuments: [
      { id: "PRO-GOOG-26-001", type: "proposal", label: "Project Proposal", lastModified: "2026-01-05" },
      { id: "AGR-GOOG-26-001", type: "agreement", label: "Service Agreement", lastModified: "2026-01-10" },
      { id: "SRS-GOOG-26-001", type: "srs", label: "System Requirements", lastModified: "2026-01-15" },
    ]
  };
};
            ...
            <Suspense fallback={
              ...
              {['documents', 'financials', 'activity'].includes(activeTab) && (
                <div className="bg-white border border-[#E2E8F0] border-dashed rounded-3xl p-20 flex flex-col items-center justify-center text-center">
                   ...
                   <h3 className="text-lg font-bold text-[#0F172A] uppercase tracking-widest">{activeTab} Workspace</h3>
                   <p className="text-sm text-[#64748B] max-w-xs mt-2">
                      The {activeTab} orchestration layer for {project.id} is currently being calibrated by the Ops team.
                   </p>
                </div>
              )}
            </Suspense>
