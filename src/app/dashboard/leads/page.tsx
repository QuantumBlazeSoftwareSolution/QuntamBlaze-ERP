...
import { MOCK_LEADS, LeadStatus } from "@/lib/mockData/leads";
...
export default function LeadsPage() {
  ...
  const stats = [
    ...
    {
      label: "New Leads",
      value: MOCK_LEADS.filter(l => l.status === "New").length,
      icon: UserPlus,
      color: "text-accent",
      bg: "bg-accent/10",
    },
    {
      label: "Qualified",
      value: MOCK_LEADS.filter(l => l.status === "Qualified").length,
      icon: CheckCircle2,
    ...
    {
      label: "Converted (Won)",
      value: MOCK_LEADS.filter(l => l.status === "Won").length,
      icon: TrendingUp,
      color: "text-success",
      bg: "bg-success/10",
    },
    ...
    <div className="space-y-8 pb-12">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-text-secondary mt-1 text-lg">Track and nurture potential business opportunities.</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {stats.map((stat, i) => (
          ...
          >
            ...
            <div>
              <p className="text-[11px] font-bold text-text-muted uppercase tracking-[0.1em] mb-0.5">{stat.label}</p>
      ...
      >
        <LeadsTable statusFilter={statusFilter} />
      </motion.div>
    </div>
  );
}
