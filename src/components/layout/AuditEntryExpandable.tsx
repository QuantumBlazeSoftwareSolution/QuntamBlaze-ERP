    ...
    <div className="relative group border-b border-[#0F0F0F] last:border-none">
      ...
      >
        ...
        <div className="relative flex flex-col items-center">
           <div className={cn("w-2.5 h-2.5 rounded-full z-10 shrink-0 mt-1.5", ACTION_COLORS[entry.action])} />
        </div>
           ...
           <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                 <div className="w-6 h-6 rounded-full bg-white/10 flex items-center justify-center overflow-hidden border border-border">
                    {entry.userAvatar ? <img src={entry.userAvatar} className="w-full h-full object-cover" /> : <User className="w-3 h-3 text-text-muted" />}
                 </div>
                 <span className="text-[13px] font-bold text-text-primary">{entry.userName}</span>
                 <span className="text-text-muted text-[12px]">•</span>
                 <span className="text-[11px] font-bold text-text-muted uppercase tracking-widest">{entry.action}</span>
              </div>
                ...
                <table className="w-full text-left border-collapse">
                   <thead>
                      <tr className="border-b border-border/20">
                         <th className="pb-3 text-[10px] font-bold text-text-muted uppercase tracking-widest">Field</th>
                         <th className="pb-3 text-[10px] font-bold text-text-muted uppercase tracking-widest">Old Value</th>
                         <th className="pb-3 text-[10px] font-bold text-text-muted uppercase tracking-widest">New Value</th>
                      </tr>
                   </thead>
                   <tbody>
                      {entry.diffs.map((diff, i) => (
                        <tr key={i} className="border-b border-border/10 last:border-none">
                           <td className="py-3 text-[12px] font-mono text-text-secondary">{diff.field}</td>
                           <td className="py-3 text-[12px] font-mono text-[#FF444499] line-through">{diff.oldValue}</td>
