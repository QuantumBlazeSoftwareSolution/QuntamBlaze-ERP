...
export function LeadsTable({ statusFilter }: { statusFilter: LeadStatus | "all" }) {
  ...
  const columns = useMemo(() => [
    columnHelper.accessor("id", {
      header: ({ column }) => (
        <button className="flex items-center gap-1.5 hover:text-accent transition-colors" onClick={() => column.toggleSorting()}>
    ...
    columnHelper.accessor("company", {
      ...
      cell: (info) => <span className="text-[13px] font-bold text-text-primary">{info.getValue()}</span>,
    }),
    ...
    columnHelper.accessor("status", {
      header: "Status",
      cell: (info) => (
        <span className={cn("text-[11px] font-bold px-2.5 py-1 rounded-full border", STATUS_CHIP[info.getValue()])}>
          {info.getValue()}
        </span>
      ),
    }),
              ...
              {table.getHeaderGroups().map((hg) => (
                <tr key={hg.id} className="border-b border-divider bg-page-bg">
                  {hg.headers.map((header) => (
                    <th key={header.id} className="px-5 py-3.5 text-left text-[10px] font-bold text-text-muted uppercase tracking-[0.15em] whitespace-nowrap">
