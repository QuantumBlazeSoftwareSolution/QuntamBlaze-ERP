...
export function ClientsTable({ data }: { data: Client[] }) {
      ...
      columnHelper.accessor("name", {
        ...
        cell: (info) => (
          <div className="flex items-center gap-3">
            ...
            <span className="text-[14px] font-bold text-text-primary">{info.getValue()}</span>
          </div>
        ),
      }),
        ...
        cell: (info) => {
          ...
          return (
            <div className="flex items-center gap-2">
              ...
              {projects.length === 0 && <span className="text-[11px] text-text-muted">0</span>}
            </div>
          );
        },
      }),
          ...
          >
            <div
              className={cn(
                ...
                info.getValue() === "Active" ? "bg-success" : "bg-text-muted"
              )}
            />
            {info.getValue()}
          </div>
  ...
  return (
    <div className="w-full overflow-hidden rounded-xl border border-border bg-white shadow-sm">
      ...
      <ClientDetailsSheet client={selectedClient} onClose={() => setSelectedClient(null)} />
    </div>
  );
}
