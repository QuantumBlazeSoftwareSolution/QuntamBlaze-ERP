      ...
      <div className="flex flex-col md:flex-row items-start justify-between gap-8">
        <div className="flex flex-col md:flex-row items-center gap-8">
          ...
          <div className="space-y-4">
            <div className="flex flex-wrap items-center gap-3">
              <IDChip id={client.id} className="text-[12px] px-3 py-1.5 bg-accent/5 border-accent/20 text-accent" />
              <span className={cn(
                "px-3 py-1.5 rounded-full text-[10px] font-bold uppercase tracking-widest border",
                client.status === "Active" ? "border-success/30 text-success bg-success/5" : "border-text-muted/30 text-text-muted bg-white/5"
              )}>
                <span className={cn("inline-block w-1.5 h-1.5 rounded-full mr-1.5", client.status === "Active" ? "bg-success shadow-[0_0_8px_rgba(0,200,150,0.5)]" : "bg-text-muted")} />
                {client.status}
              </span>
            </div>
        ...
        <div className="flex flex-col items-end gap-4">
          <div className="bg-bg-primary/50 border border-border rounded-xl p-4 flex items-center gap-4">
            ...
            <div>
              <p className="text-[10px] font-bold text-text-muted uppercase tracking-[0.15em]">Account Manager</p>
              <p className="text-[14px] font-bold text-text-primary">{client.accountManager.name}</p>
            </div>
          </div>
