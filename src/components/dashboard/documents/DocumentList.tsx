      ...
      <div className="p-8 space-y-6">
        <div className="flex items-center justify-between mb-8">
           ...
           <div className="flex gap-4">
              <button className="p-2 text-text-muted hover:text-accent"><ChevronRight className="w-4 h-4 rotate-90" /></button>
              <button className="p-2 text-text-muted hover:text-accent"><ChevronRight className="w-4 h-4 rotate-[-90deg]" /></button>
           </div>
        </div>
              ...
              {documents.map((doc) => (
                ...
                >
                  ...
                  <div className="col-span-2">
                     <IDChip id={doc.id.split('-').slice(0, 2).join('-')} className="bg-transparent border-none p-0 text-text-muted font-mono" />
                  </div>
                  <div className="col-span-2 flex items-center gap-2">
                     <div className="w-6 h-6 rounded-full bg-accent/10 border border-accent/20 flex items-center justify-center text-[8px] font-bold text-accent">
                        {doc.authorName.split(' ').map(n => n[0]).join('')}
                     </div>
