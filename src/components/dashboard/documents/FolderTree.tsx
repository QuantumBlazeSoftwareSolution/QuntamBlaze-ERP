...
function FolderItem({ node, depth = 0 }: { node: FolderNode; depth?: number }) {
  ...
  return (
    <div className="select-none">
      ...
      {isFolder &&
        isOpen &&
        node.children?.map((child) => <FolderItem key={child.id} node={child} depth={depth + 1} />)}
    </div>
  );
}
...
export function FolderTree({ tree }: { tree: FolderNode[] }) {
  return (
    <div className="w-[260px] bg-white border-r border-divider h-full overflow-y-auto py-6 shadow-sm">
      <h3 className="px-6 text-[11px] font-bold text-text-muted uppercase tracking-[0.2em] mb-6">
        Project Directories
      </h3>
