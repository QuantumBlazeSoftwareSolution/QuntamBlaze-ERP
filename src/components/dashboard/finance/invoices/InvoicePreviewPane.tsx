      ...
      >
        ...
        <div className="flex justify-between mb-16">
          <div className="space-y-6">
            <div>
              <p className="text-[11px] font-black text-[#8A8A8A] uppercase tracking-widest mb-2">Billed To</p>
              ...
              <p className="text-[13px] text-[#8A8A8A] max-w-[200px] mt-2 leading-relaxed">{data.billingAddress || "—"}</p>
            </div>
          </div>
          <div className="text-right space-y-6">
            <div>
              <p className="text-[11px] font-black text-[#8A8A8A] uppercase tracking-widest mb-1">Issue Date</p>
              <p className="text-[15px] font-bold text-[#050505]">{data.issueDate}</p>
            </div>
            <div>
              <p className="text-[11px] font-black text-[#8A8A8A] uppercase tracking-widest mb-1">Due Date</p>
          ...
          <table className="w-full border-t border-b border-[#050505] mb-8">
            <thead>
              <tr className="bg-[#F9FAFB]">
                <th className="text-left py-4 px-2 text-[11px] font-black text-[#8A8A8A] uppercase tracking-widest">Description</th>
                <th className="text-center py-4 px-2 text-[11px] font-black text-[#8A8A8A] uppercase tracking-widest">Qty</th>
                <th className="text-right py-4 px-2 text-[11px] font-black text-[#8A8A8A] uppercase tracking-widest">Rate</th>
                <th className="text-center py-4 px-2 text-[11px] font-black text-[#8A8A8A] uppercase tracking-widest">Tax</th>
                <th className="text-right py-4 px-2 text-[11px] font-black text-[#8A8A8A] uppercase tracking-widest">Amount</th>
              </tr>
            </thead>
            <tbody>
              {data.lineItems.map((item, index) => (
                <tr key={index} className="border-b border-[#E5E7EB]">
                  <td className="py-5 px-2 text-[14px] text-[#050505] font-medium">{item.description || "—"}</td>
                  <td className="py-5 px-2 text-[14px] text-center text-[#3A3A3A]">{item.qty}</td>
                  <td className="py-5 px-2 text-[14px] text-right text-[#3A3A3A]">{item.rate.toLocaleString()}</td>
                  <td className="py-5 px-2 text-[14px] text-center text-[#8A8A8A]">{item.taxPercent}%</td>
        ...
        <div className="mt-auto pt-12 border-t border-[#E5E7EB] grid grid-cols-2 gap-12">
           <div className="space-y-4">
              <p className="text-[11px] font-black text-[#8A8A8A] uppercase tracking-widest">Bank Details</p>
              <div className="text-[12px] text-[#3A3A3A] space-y-1">
                 <p><span className="text-[#8A8A8A]">Bank:</span> Quantum Global Finance</p>
                 <p><span className="text-[#8A8A8A]">IBAN:</span> AE02 4000 0000 1234 5678 901</p>
                 <p><span className="text-[#8A8A8A]">SWIFT:</span> QBZAAEAD</p>
              </div>
           </div>
           <div className="flex flex-col items-end justify-end">
              <div className="w-48 h-20 border-2 border-dashed border-[#E5E7EB] rounded-lg flex items-center justify-center">
                 <span className="text-[10px] font-bold text-[#8A8A8A] uppercase tracking-widest">Awaiting Signature</span>
              </div>
           </div>
        </div>
      </motion.div>
