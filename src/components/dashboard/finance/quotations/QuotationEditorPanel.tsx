...
export function QuotationEditorPanel({ form }: QuotationEditorPanelProps) {
  ...
  const { fields: milestones, append: addMilestone, remove: removeMilestone } = useFieldArray({
    control,
    name: "milestones"
  });
      ...
      <div className="max-w-3xl mx-auto space-y-10">
        <div className="flex items-center justify-between">
           ...
           <div className="px-3 py-1.5 rounded-lg bg-accent/10 border border-accent/30 flex items-center gap-2">
             ...
             <span className="text-[11px] font-mono font-bold text-accent">{watch("quotationId")}</span>
           </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-6">
          <div className="space-y-4">
            <label className="text-[11px] font-bold text-text-muted uppercase tracking-widest">Client / Lead Reference</label>
          ...
          <div className="space-y-4">
            <label className="text-[11px] font-bold text-text-muted uppercase tracking-widest">Validity Horizon</label>
        ...
        {/* Scope of Operations */}
        <div className="space-y-4">
          <label className="text-[11px] font-bold text-text-muted uppercase tracking-widest">Scope of Operations</label>
          <div className="bg-bg-card border border-border rounded-xl overflow-hidden focus-within:border-accent/50 transition-all">
             <div className="px-4 py-2 border-b border-border bg-white/5 flex gap-4">
                <button className="p-1 text-text-muted hover:text-accent transition-colors font-bold">B</button>
                <button className="p-1 text-text-muted hover:text-accent transition-colors italic">I</button>
                <button className="p-1 text-text-muted hover:text-accent transition-colors">List</button>
             </div>
             <textarea 
        ...
        <div className="space-y-6">
          <div className="flex items-center justify-between">
            <label className="text-[11px] font-bold text-text-muted uppercase tracking-widest">Resource Allocation (Line Items)</label>
          </div>
          <LineItemsTable control={control} register={register} watch={watch} showHoursColumn={true} />
        </div>
        ...
        <div className="space-y-6 pt-6 border-t border-border/50">
           <div className="flex items-center justify-between">
              <div className="flex flex-col">
                 <label className="text-[11px] font-bold text-text-primary uppercase tracking-widest">Milestone Structure</label>
                 <span className="text-[11px] text-text-muted mt-1">Break project total into payment tranches</span>
              </div>
           ...
           {useMilestones && (
             ...
             >
                {milestones.map((milestone, index) => (
                  <div key={milestone.id} className="flex gap-4 items-center bg-accent/[0.03] border-l-2 border-accent/20 p-4 rounded-r-xl">
                     <input 
                     ...
                     <button onClick={() => removeMilestone(index)} className="p-1.5 text-text-muted hover:text-danger">
                        <Trash2 className="w-4 h-4" />
                     </button>
                  </div>
                ))}
                <button 
                  onClick={() => addMilestone({ id: Math.random().toString(), description: "", percentage: 50, amount: 0 })}
           ...
           <div className="bg-bg-card border border-border rounded-xl p-6 w-80 space-y-6">
              ...
              <div className="flex justify-between items-center">
                 <div className="flex items-center gap-3">
                    <span className="text-[12px] text-text-muted uppercase tracking-widest">Discount</span>
                    <input 
                       type="number"
              ...
              <div className="flex justify-between items-end pt-4 border-t border-border/50">
                 <span className="text-[12px] font-black text-text-muted uppercase tracking-widest">TOTAL</span>
