      ...
      >
        ...
        <div className="flex justify-between items-start mb-16">
          <div className="space-y-4">
            <h1 className="text-5xl font-black tracking-tighter text-[#050505] leading-none">
              QUANTUM
              <br />
              BLAZE
            </h1>
            <p className="text-[12px] font-bold text-[#8A8A8A] uppercase tracking-widest">
              Systems Engineering Div.
            </p>
          </div>
          <div className="text-right">
            <h2 className="text-3xl font-black text-[#00E5FF] tracking-tighter uppercase mb-2">
              QUOTATION
            </h2>
        ...
        {/* Scope */}
        <div className="mb-12">
          <p className="text-[10px] font-black text-[#8A8A8A] uppercase tracking-widest mb-4">
            SCOPE OF OPERATIONS
          </p>
          <p className="text-[14px] text-[#3A3A3A] leading-relaxed whitespace-pre-wrap">
            {data.scopeOfWork ||
              "Initial deployment and architectural design for Nexus Corp core processing units. This includes load balancing configurations and security protocol integration across all active nodes."}
          </p>
        </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-[#050505] text-white">
                ...
                <th className="text-center py-3 px-4 text-[10px] font-black uppercase tracking-widest">
                  Hrs/Qty
                </th>
            ...
            <tbody>
              {data.lineItems.map((item, index) => (
                <tr key={index} className="border-b border-[#E5E7EB]">
                  <td className="py-5 px-4 text-[14px] text-[#050505] font-bold">
                    {item.description || "—"}
                  </td>
                  <td className="py-5 px-4 text-[14px] text-center text-[#3A3A3A]">
                    {item.hours || item.qty}
                  </td>
                  <td className="py-5 px-4 text-[14px] text-right text-[#3A3A3A]">
        ...
        {data.useMilestones && data.milestones.length > 0 && (
          <div className="mt-8 border-t border-[#E5E7EB] pt-12">
            <p className="text-[10px] font-black text-[#8A8A8A] uppercase tracking-widest mb-6">
              PROPOSED MILESTONES
            </p>
            <div className="grid grid-cols-1 gap-4">
              {data.milestones.map((ms, i) => (
                <div
                  key={i}
                  className="flex justify-between items-center p-4 bg-[#F9FAFB] rounded border-l-4 border-[#00E5FF]"
                >
                  <span className="text-[14px] font-bold text-[#050505]">
                    {ms.description || `Milestone ${i + 1}`}
                  </span>
                  <div className="flex items-center gap-8">
                    <span className="text-[13px] text-[#8A8A8A] font-bold">{ms.percentage}%</span>
                    <span className="text-[14px] font-black text-[#050505]">
                      {((total * ms.percentage) / 100).toLocaleString()}
                    </span>
                  </div>
                </div>
              ))}
