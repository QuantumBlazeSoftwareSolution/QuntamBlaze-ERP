  ...
  return (
    <div className="bg-white border border-[#E2E8F0] rounded-3xl p-8 shadow-sm h-full flex flex-col">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
        <div>
          ...
          <p className="text-[10px] text-[#94A3B8] font-bold uppercase tracking-widest">
            Calculations for May 2024 Cycle
          </p>
        </div>
          ...
          <button
            ...
            className={cn(
              ...
              isProcessing
                ...
                : "bg-blue-600 shadow-blue-600/20 hover:scale-[1.02]"
            )}
          >
            {isProcessing ? (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 mb-8">
        ...
        />
        <StatusCard
          icon={<FileCheck className="w-4 h-4 text-[#10B981]" />}
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="border-b border-[#F1F5F9] bg-white">
              <th className="px-6 py-4 text-[10px] font-black text-[#94A3B8] uppercase tracking-widest">
                Employee
              </th>
              <th className="px-6 py-4 text-[10px] font-black text-[#94A3B8] uppercase tracking-widest">
                Structure
              </th>
              <th className="px-6 py-4 text-[10px] font-black text-[#94A3B8] uppercase tracking-widest text-right">
                Basic
              </th>
              <th className="px-6 py-4 text-[10px] font-black text-[#94A3B8] uppercase tracking-widest text-right">
                Allowances
              </th>
              <th className="px-6 py-4 text-[10px] font-black text-red-400 uppercase tracking-widest text-right">
                LOP (Unpaid)
              </th>
              <th className="px-6 py-4 text-[10px] font-black text-[#0F172A] uppercase tracking-widest text-right">
                Net Payable
              </th>
            </tr>
          </thead>
          <tbody>
            {MOCK_ITEMS.map((item) => (
              <tr
                key={item.id}
                className="border-b border-[#F1F5F9] hover:bg-white transition-colors group"
              >
                ...
                <td className="px-6 py-4 text-xs font-bold text-[#475569] text-right">
                  {item.basic}
                </td>
                <td className="px-6 py-4 text-xs font-bold text-[#10B981] text-right">
                  +{item.allowances}
                </td>
                <td className="px-6 py-4 text-xs font-bold text-red-500 text-right">-{item.lop}</td>
                <td className="px-6 py-4 text-xs font-black text-[#0F172A] text-right">
                  {item.net}
                </td>
              </tr>
            ))}
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-[10px] text-blue-700 font-medium">
              All attendance records (ATT-ID) have been cross-checked for the current cycle.
            </p>
          </div>
        </div>
