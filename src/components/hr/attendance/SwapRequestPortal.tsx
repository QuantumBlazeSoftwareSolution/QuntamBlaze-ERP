    ...
    <div className="bg-white border border-[#E2E8F0] rounded-3xl p-6 shadow-sm h-full flex flex-col">
      <div className="flex items-center justify-between mb-8">
        <div>
           ...
           <p className="text-[10px] text-[#94A3B8] font-bold uppercase tracking-widest mt-1">Pending Peer-to-Peer Requests</p>
        </div>
      ...
      <div className="flex-1 space-y-4 overflow-auto scrollbar-hide">
         {MOCK_SWAPS.map((swap) => (
           <div key={swap.id} className="p-5 rounded-2xl border border-[#F1F5F9] bg-[#F8FAFC] hover:border-blue-200 transition-all">
              ...
              <div className="flex items-center justify-between mb-6">
                 <div className="flex-1">
                    <p className="text-[9px] font-black text-[#94A3B8] uppercase tracking-widest mb-1">Requestor</p>
                    <p className="text-xs font-bold text-[#0F172A]">{swap.requestor.name}</p>
                    <p className="text-[9px] font-bold text-blue-600 uppercase">{swap.requestor.shift} Shift</p>
                 </div>
                 <div className="px-4">
                    <ArrowRightLeft className="w-5 h-5 text-[#CBD5E1]" />
                 </div>
                 <div className="flex-1 text-right">
                    <p className="text-[9px] font-black text-[#94A3B8] uppercase tracking-widest mb-1">Target Partner</p>
                    <p className="text-xs font-bold text-[#0F172A]">{swap.target.name}</p>
                    <p className="text-[9px] font-bold text-slate-800 uppercase">{swap.target.shift} Shift</p>
                 </div>
              </div>
              ...
              <div className="p-3 rounded-xl bg-white border border-[#F1F5F9] mb-6">
                 ...
                 <p className="text-[11px] text-[#475569] leading-relaxed font-medium">{swap.reason}</p>
              </div>
