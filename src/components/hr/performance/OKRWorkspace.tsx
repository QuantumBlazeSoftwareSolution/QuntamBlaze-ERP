...
const MOCK_GOALS = [
  { 
    ...
    progress: 65,
    results: [
      { title: 'Migrate 100% Core Tables to Drizzle', current: 85, target: 100, unit: '%' },
      { title: 'Reduce API Latency by 200ms', current: 120, target: 200, unit: 'ms' },
    ]
  },
  { 
    ...
    progress: 40,
    results: [
      { title: 'Weekly 1-on-1 Sessions', current: 12, target: 24, unit: 'sessions' },
      { title: 'Skills Mastery Checklist Completion', current: 1, target: 3, unit: 'mentees' },
    ]
  },
];
    ...
    <div className="bg-white border border-[#E2E8F0] rounded-3xl p-8 shadow-sm h-full flex flex-col">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
        <div>
           ...
           <p className="text-[10px] text-[#94A3B8] font-bold uppercase tracking-widest mt-1">Strategic Alignment — FY 2024</p>
        </div>
      ...
      <div className="flex-1 space-y-8 overflow-auto scrollbar-hide">
         {MOCK_GOALS.map((goal) => (
           <div key={goal.id} className="p-8 rounded-3xl border border-[#F1F5F9] bg-[#F8FAFC] hover:border-blue-200 transition-all group">
              <div className="flex items-start justify-between mb-8">
                 ...
                 <div className="flex flex-col items-end">
                    ...
                    <span className="text-[9px] font-black text-[#94A3B8] uppercase tracking-widest">Total Progress</span>
                 </div>
              </div>
              ...
              <div className="space-y-4">
                 <h5 className="text-[10px] font-black text-[#64748B] uppercase tracking-[0.2em] mb-4">Key Results</h5>
                 {goal.results.map((kr, idx) => (
                   <div key={idx} className="p-4 rounded-2xl bg-white border border-[#F1F5F9] group-hover:border-blue-100 transition-all">
                      <div className="flex items-center justify-between mb-3">
                         <span className="text-xs font-bold text-[#475569]">{kr.title}</span>
                         <span className="text-[10px] font-black text-[#0F172A]">
                            {kr.current} / {kr.target} <span className="text-[#94A3B8] ml-1">{kr.unit}</span>
                         </span>
                      </div>
