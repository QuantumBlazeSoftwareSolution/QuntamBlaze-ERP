  ...
  return (
    <div className="bg-white border border-[#E2E8F0] rounded-xl overflow-hidden">
      <div className="p-6 border-b border-[#F1F5F9] flex items-center justify-between">
        ...
        <span className="text-[#94A3B8] text-[11px] font-semibold uppercase tracking-widest">Real-time</span>
      </div>

      <div className="divide-y divide-[#F1F5F9]">
        {MOCK_HR_ACTIVITIES.map((activity) => {
          const config = iconMap[activity.type] || { icon: FileText, color: "text-gray-600 bg-gray-50" };
            ...
            <div key={activity.id} className="p-5 hover:bg-[#F8FAFC] transition-colors">
              <div className="flex gap-4">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-[11px] text-[#94A3B8] mt-1.5 font-medium">{activity.timestamp}</p>
                </div>
              </div>
            </div>
          );
