  ...
  return (
    <div className="bg-white border border-[#E2E8F0] rounded-2xl p-6 h-full shadow-sm flex flex-col">
      ...
      <div className="space-y-4 flex-1">
        {UPCOMING.map((int) => (
          <div key={int.id} className="p-4 rounded-xl border border-[#F1F5F9] bg-[#F8FAFC]/50 hover:bg-white hover:border-[#E2E8F0] hover:shadow-md transition-all group cursor-pointer">
            <div className="flex items-start justify-between mb-3">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-full bg-slate-200 border-2 border-white shadow-sm overflow-hidden shrink-0">
                   {int.avatar ? <img src={int.avatar} alt="" className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center font-bold text-[#94A3B8]">{int.candidate[0]}</div>}
                </div>
                <div>
                  <h4 className="text-sm font-bold text-[#0F172A] group-hover:text-[#10B981] transition-colors">{int.candidate}</h4>
                  <p className="text-[10px] text-[#94A3B8] font-medium truncate max-w-[120px]">{int.role}</p>
                </div>
              </div>
            ...
            <div className="flex flex-wrap items-center gap-3 mb-4">
               ...
               <span className={cn(
                 ...
                 int.type === 'Technical' ? "bg-violet-50 text-violet-600 border-violet-100" : "bg-emerald-50 text-emerald-600 border-emerald-100"
               )}>
                 {int.type}
               </span>
            </div>
