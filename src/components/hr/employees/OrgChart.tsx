  ...
  children: [
    {
      ...
      role: "HR Manager",
      children: [
         { id: "EMP-HR-26-012", name: "Elena Vance", role: "HR Specialist" }
      ]
    },
    {
      id: "EMP-ENG-26-001",
    ...
    <div className="flex flex-col items-center">
       <motion.div 
         ...
         className={cn(
           ...
           isRoot ? "bg-[#0F172A] border-[#0F172A] text-white" : "bg-white border-[#F1F5F9] hover:border-blue-400"
         )}
       >
          <div className="flex items-center gap-3">
             <div className="w-10 h-10 rounded-xl overflow-hidden border-2 border-white/20">
                <img src={node.avatar || "https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?q=80&w=100&auto=format&fit=crop"} alt={node.name} className="w-full h-full object-cover" />
             </div>
             <div>
                <p className={cn("text-xs font-black truncate max-w-[120px]", isRoot ? "text-white" : "text-[#0F172A]")}>{node.name}</p>
                <p className={cn("text-[9px] font-bold uppercase tracking-widest", isRoot ? "text-blue-300" : "text-[#94A3B8]")}>{node.role}</p>
             </div>
          </div>
       </motion.div>
