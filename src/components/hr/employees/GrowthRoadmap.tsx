...
const MILESTONES = [
  { id: 1, title: 'Senior Software Engineer', status: 'completed', date: 'Jan 2024', current: false },
  { id: 2, title: 'Lead Engineer Track', status: 'in_progress', date: 'Estimated Q3 2024', current: true },
  { id: 3, title: 'Staff Engineer / Architect', status: 'upcoming', date: 'Goal 2025', current: false },
  { id: 4, title: 'Engineering Director', status: 'upcoming', date: 'Long-term Goal', current: false },
];
    ...
    <div className="bg-white border border-[#E2E8F0] rounded-3xl p-8 shadow-sm h-full flex flex-col">
      <div className="flex items-center justify-between mb-10">
        <div>
           ...
           <p className="text-[10px] text-[#94A3B8] font-bold uppercase tracking-widest mt-1">Strategic Path to Leadership</p>
        </div>
         ...
         {MILESTONES.map((ms, idx) => (
           <div key={ms.id} className="relative pl-12 pb-10 last:pb-0">
              <div className={cn(
                ...
                ms.status === 'completed' ? "bg-[#10B981] shadow-lg shadow-[#10B981]/20" : 
                ms.status === 'in_progress' ? "bg-blue-500 shadow-lg shadow-blue-500/20" : "bg-[#CBD5E1]"
              )}>
                 {ms.status === 'completed' ? <CheckCircle2 className="w-4 h-4 text-white" /> : 
                  ms.status === 'in_progress' ? <Zap className="w-4 h-4 text-white animate-pulse" /> : <Circle className="w-4 h-4 text-white" />}
              </div>

              <div className={cn(
                "p-4 rounded-2xl border transition-all",
                ms.current ? "bg-[#F8FAFC] border-blue-100 shadow-sm" : "bg-white border-transparent"
              )}>
