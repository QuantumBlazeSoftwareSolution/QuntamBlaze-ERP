        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 mt-1">
               ...
               <span className="text-[#94A3B8] text-[10px] uppercase font-bold tracking-wider">• Round 2 • May 10, 2024</span>
            </div>
          </div>
        </div>
          ...
          <div className="bg-white border border-[#E2E8F0] rounded-xl p-8 shadow-sm">
            ...
            <div className="mt-8 space-y-4">
              <div className="space-y-2">
                 <label className="text-[11px] font-bold text-[#94A3B8] uppercase tracking-widest px-1">Executive Summary</label>
                 <textarea 
        ...
        <div className="space-y-6">
          <div className="bg-white border border-[#E2E8F0] rounded-xl p-8 sticky top-24 shadow-md">
             <div className="text-center mb-8">
                <p className="text-[10px] font-bold text-[#94A3B8] uppercase tracking-[0.2em] mb-2">Aggregate Score</p>
                <div className="relative inline-flex items-center justify-center">
                  <span className={cn(
                    "text-6xl font-black transition-colors duration-500",
                    overallScore > 80 ? "text-[#10B981]" : overallScore > 60 ? "text-amber-500" : overallScore > 0 ? "text-red-500" : "text-[#E2E8F0]"
                  )}>
                    {overallScore || '--'}
                  </span>
             ...
             <div className="mt-10 space-y-3">
                <button className="w-full py-3.5 rounded-xl bg-[#10B981] text-white font-bold text-sm shadow-lg shadow-[#10B981]/20 hover:scale-[1.02] active:scale-[0.98] transition-all disabled:opacity-50 disabled:scale-100" disabled={!overallScore || !recommendation}>
                  Submit Scorecard
                </button>
             ...
             <div className="mt-6 flex items-start gap-2 p-3 bg-blue-50/50 rounded-lg border border-blue-100">
                ...
                <p className="text-[10px] text-blue-700 leading-normal">
                  All scores are weighted based on the job requirement profile. Minimum of 4 stars across technical criteria is recommended for a 'Hire'.
                </p>
             </div>
          </div>
        </div>
  ...
  return (
    <div className="bg-white border border-[#E2E8F0] rounded-xl overflow-hidden shadow-sm">
      ...
      <div className="divide-y divide-[#F1F5F9]">
        {criteria.map((c: any) => (
          <div key={c.id} className="p-6 flex flex-col md:flex-row md:items-center justify-between gap-6 group hover:bg-[#F8FAFC] transition-colors">
            <div className="flex-1">
              <div className="flex items-center gap-2 mb-1">
                <p className="text-sm font-bold text-[#475569]">{c.label}</p>
                <span className="text-[10px] text-[#94A3B8] font-medium">({Math.round(c.weight * 100)}%)</span>
              </div>
              <textarea 
              ...
              {[1, 2, 3, 4, 5].map(star => (
                <button
                  ...
                  className={cn(
                    ...
                    ratings[c.id] >= star ? "text-amber-500 scale-110" : "text-[#CBD5E1] hover:text-amber-300"
                  )}
                >
...
function RecommendationCard({ type, active, onClick }: any) {
  const configs: any = {
    strong_hire: { label: 'Strong Hire', color: 'text-[#10B981]', bg: 'bg-[#ECFDF5]', border: 'border-[#A7F3D0]', icon: CheckCircle2 },
    hire: { label: 'Hire', color: 'text-[#3B82F6]', bg: 'bg-[#EFF6FF]', border: 'border-[#BFDBFE]', icon: CheckCircle2 },
    no_hire: { label: 'No Hire', color: 'text-[#F59E0B]', bg: 'bg-[#FFFBEB]', border: 'border-[#FDE68A]', icon: AlertCircle },
    strong_no_hire: { label: 'Strong No Hire', color: 'text-[#EF4444]', bg: 'bg-[#FEF2F2]', border: 'border-[#FECACA]', icon: AlertCircle },
  };
  ...
  return (
    <button
      ...
      className={cn(
        ...
        active ? `${config.bg} ${config.border} ring-2 ring-offset-2 ring-slate-100` : "bg-white border-[#E2E8F0] hover:border-[#CBD5E1]"
      )}
    >
