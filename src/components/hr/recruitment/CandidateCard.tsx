...
export function CandidateCard({ candidate }: CandidateCardProps) {
  ...
  return (
    ...
    >
      ...
      <h4 className="text-[#0F172A] text-sm font-semibold mb-1 group-hover:text-[#10B981] transition-colors">
        {candidate.name}
      </h4>
        ...
        <div className="flex items-center gap-1.5 text-[#64748B]">
          ...
          <span className="text-[10px] font-medium">
            {candidate.nextInterviewDate
              ? `Next: ${candidate.nextInterviewDate}`
              : `D+${candidate.daysInStage}`}
          </span>
        </div>

        {candidate.assignee ? (
          <div
            className="w-6 h-6 rounded-full bg-blue-100 flex items-center justify-center border border-white shadow-sm"
            title={candidate.assignee.name}
          >
