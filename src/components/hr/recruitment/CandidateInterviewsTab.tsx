    ...
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        {interviews.map((int, idx) => (
          <div key={int.id} className="bg-white border border-[#E2E8F0] rounded-xl overflow-hidden group hover:shadow-md transition-all">
            <div className="p-6 flex flex-col md:flex-row md:items-center justify-between gap-6">
              <div className="flex items-start gap-5">
                <div className={cn(
                  "w-12 h-12 rounded-xl flex items-center justify-center shrink-0 border",
                  int.status === 'completed' ? "bg-green-50 text-[#10B981] border-green-100" : "bg-blue-50 text-blue-600 border-blue-100"
                )}>
                  {int.status === 'completed' ? <CheckCircle2 className="w-6 h-6" /> : <Calendar className="w-6 h-6" />}
                </div>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap items-center gap-y-2 gap-x-5 text-[11px] text-[#64748B] font-medium">
                    ...
                    <div className="flex items-center gap-1.5">
                      {int.mode.includes('Video') ? <Video className="w-3.5 h-3.5" /> : <MapPin className="w-3.5 h-3.5" />}
                      {int.mode}
                    </div>
                  </div>
                </div>
              ...
              <div className="flex flex-col md:items-end gap-3">
                <div className="flex -space-x-2">
                  {int.interviewers.map((name, i) => (
                    <div key={i} className="w-8 h-8 rounded-full bg-slate-100 border-2 border-white flex items-center justify-center text-[10px] font-bold text-[#64748B] shadow-sm" title={name}>
                      {name[0]}
                    </div>
                  ))}
                  <div className="w-8 h-8 rounded-full bg-slate-50 border-2 border-white border-dashed flex items-center justify-center text-[10px] text-[#94A3B8]" title="Add Interviewer">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <button className={cn(
                    ...
                    int.hasScorecard ? "bg-[#F8FAFC] text-[#475569] border border-[#E2E8F0] hover:bg-[#F1F5F9]" : "bg-[#10B981] text-white shadow-md shadow-[#10B981]/10 hover:scale-[1.02]"
                  )}>
                    {int.hasScorecard ? <FileSearch className="w-3.5 h-3.5" /> : <PlusCircle className="w-3.5 h-3.5" />}
