    ...
    <div className="bg-white border border-[#E2E8F0] rounded-3xl p-8 shadow-sm flex flex-col h-full">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
        <div>
           ...
           <p className="text-[10px] text-[#94A3B8] font-bold uppercase tracking-widest mt-1">Workforce Availability — May 2024</p>
        </div>
         ...
         <div className="grid grid-cols-7 gap-px bg-[#F1F5F9] border border-[#F1F5F9] rounded-2xl overflow-hidden">
            {WEEKDAYS.map(day => (
              <div key={day} className="bg-[#F8FAFC] py-3 text-center">
                 <span className="text-[10px] font-black text-[#94A3B8] uppercase tracking-widest">{day}</span>
              </div>
            ))}
                 ...
                 >
                    <div className="flex items-center justify-between mb-1">
                       <span className={cn(
                         ...
                         holiday ? "bg-red-500 text-white shadow-sm" : 
                         isWeekend ? "text-[#94A3B8]" : "text-[#0F172A]"
                       )}>
                          {day}
                       </span>
                       {holiday && (
                       ...
                       {leaves.map((l, i) => (
                         <div 
                           ...
                           className={cn(
                             ...
                             l.type === 'Annual' ? "bg-blue-50 border-blue-100 text-blue-600" :
                             l.type === 'Sick' ? "bg-red-50 border-red-100 text-red-600" :
                             "bg-amber-50 border-amber-100 text-amber-600"
                           )}
                         >
                            <div className={cn(
                              "w-1 h-1 rounded-full",
                              l.type === 'Annual' ? "bg-blue-500" :
                              l.type === 'Sick' ? "bg-red-500" : "bg-amber-500"
                            )} />
                            {l.name}
                         </div>
                       ))}
      ...
      <div className="mt-8 pt-8 border-t border-[#F8FAFC] flex items-center justify-between">
         <div className="flex items-center gap-6">
            ...
            <LegendItem label="Public Holiday" color="bg-red-500" isIcon={<Gift className="w-2.5 h-2.5 text-white" />} />
         </div>
...
function LegendItem({ label, color, isIcon }: { label: string, color: string, isIcon?: React.ReactNode }) {
  return (
    <div className="flex items-center gap-2">
       <div className={cn("w-3 h-3 rounded flex items-center justify-center shadow-sm", color)}>
          {isIcon}
       </div>
       <span className="text-[9px] font-black text-[#94A3B8] uppercase tracking-tighter">{label}</span>
    </div>
  );
}
