...
const MOCK_BALANCES = [
  { type: 'Annual Leave', total: 21, taken: 5, remaining: 16, color: 'bg-blue-500', bg: 'bg-blue-50' },
  { type: 'Sick Leave', total: 14, taken: 2, remaining: 12, color: 'bg-red-500', bg: 'bg-red-50' },
  { type: 'Casual Leave', total: 7, taken: 3, remaining: 4, color: 'bg-amber-500', bg: 'bg-amber-50' },
];
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
      {MOCK_BALANCES.map((item) => {
        ...
        return (
          <div key={item.type} className="bg-white border border-[#E2E8F0] rounded-3xl p-6 shadow-sm hover:shadow-md transition-all">
             <div className="flex items-center justify-between mb-4">
                <div className={cn("w-10 h-10 rounded-xl flex items-center justify-center", item.bg)}>
                   <Calendar className={cn("w-5 h-5", item.color.replace('bg-', 'text-'))} />
                </div>
                <span className="text-[10px] font-black text-[#94A3B8] uppercase tracking-widest">Balance 2024</span>
             </div>
             ...
             <div className="flex items-end gap-2 mb-4">
                ...
                <span className="text-xs font-bold text-[#94A3B8] mb-1">/ {item.total} Days Left</span>
             </div>
