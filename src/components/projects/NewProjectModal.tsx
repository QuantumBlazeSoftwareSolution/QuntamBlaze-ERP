    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center">
      ...
      >
        ...
        <div className="px-8 py-6 border-b border-[#F1F5F9] flex items-center justify-between">
          <div>
            <h2 className="text-xl font-black text-[#0F172A] tracking-tight uppercase">Initiate New Project</h2>
            <p className="text-xs font-bold text-[#94A3B8] uppercase tracking-widest mt-1">Project Execution Framework v4.0</p>
          </div>
          <button 
            onClick={closeNewProjectModal}
          ...
          <form id="new-project-form" onSubmit={handleSubmit(onSubmit)} className="space-y-8">
              ...
              {generatedId && (
                ...
                >
                   <div className="flex items-center gap-3">
                      ...
                      <div>
                         <p className="text-[10px] font-black text-[#065F46] uppercase tracking-widest">System Generated ID</p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
               <div className="md:col-span-2 space-y-2">
                  <label className="text-[10px] font-black text-[#0F172A] uppercase tracking-widest ml-1">Project Identifier Name</label>
                  <input 
                    {...register('name')}
                  ...
                  />
                  {errors.name && <p className="text-[10px] font-bold text-red-500 mt-1 flex items-center gap-1"><AlertCircle className="w-3 h-3" /> {errors.name.message}</p>}
               </div>

               <div className="space-y-2">
                  <label className="text-[10px] font-black text-[#0F172A] uppercase tracking-widest ml-1">Client Attribution</label>
                  <select 
                    {...register('clientId')}
                  ...
                  {errors.clientId && <p className="text-[10px] font-bold text-red-500 mt-1 flex items-center gap-1"><AlertCircle className="w-3 h-3" /> {errors.clientId.message}</p>}
               </div>

               <div className="space-y-2">
                  <label className="text-[10px] font-black text-[#0F172A] uppercase tracking-widest ml-1">Engagement Model</label>
                     ...
                     {["Fixed Price", "Retainer", "T&M"].map((type) => (
                       <button
                         ...
                         className={cn(
                           ...
                           watch('type') === type ? "bg-white text-[#10B981] shadow-sm" : "text-[#94A3B8] hover:text-[#475569]"
                         )}
                       >
                         {type}
                       </button>
               ...
               <div className="space-y-2">
                  <label className="text-[10px] font-black text-[#0F172A] uppercase tracking-widest ml-1">Operational Start</label>
               ...
               <div className="space-y-2">
                  <label className="text-[10px] font-black text-[#0F172A] uppercase tracking-widest ml-1">Target Deadline</label>
                  <div className="relative">
                     <Calendar className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-[#94A3B8]" />
                     <input 
                       type="date"
               ...
               <div className="space-y-2">
                  <label className="text-[10px] font-black text-[#0F172A] uppercase tracking-widest ml-1">Budget Allocation</label>
                  <div className="relative">
               ...
               <div className="space-y-2">
                  <label className="text-[10px] font-black text-[#0F172A] uppercase tracking-widest ml-1">Project Squad</label>
                  <div className="flex flex-wrap gap-2 p-2 bg-[#F8FAFC] border border-[#E2E8F0] rounded-xl min-h-[48px]">
                     {TEAM_MEMBERS.map(member => (
                       ...
                       >
                         <div className={cn("w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold border-2 border-white", member.color)}>
                            {member.initials}
                         </div>
                  ...
                  {errors.teamMembers && <p className="text-[10px] font-bold text-red-500 mt-1">{errors.teamMembers.message}</p>}
               </div>

               <div className="md:col-span-2 space-y-2">
                  <label className="text-[10px] font-black text-[#0F172A] uppercase tracking-widest ml-1">Strategic Scope & Description</label>
                  <textarea 
                  ...
                  />
                  {errors.description && <p className="text-[10px] font-bold text-red-500 mt-1 flex items-center gap-1"><AlertCircle className="w-3 h-3" /> {errors.description.message}</p>}
               </div>
            </div>
          </form>
        </div>
