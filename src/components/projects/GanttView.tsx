...
import { getTaskWidth, getPositionFromDate, generateTimelineDays } from "@/lib/gantt/dateUtils";
import { Calendar, ChevronLeft, ChevronRight, Layers, ZoomIn, ZoomOut } from "lucide-react";
...
const MOCK_GANTT_TASKS: GanttTask[] = [
  {
    ...
    status: "Done",
    assignee: { initials: "JD", color: "bg-emerald-500" },
  },
  {
    id: "TSK-26-002",
    ...
    status: "Done",
    assignee: { initials: "AL", color: "bg-blue-500" },
    dependsOn: "TSK-26-001",
  },
  {
    id: "TSK-26-003",
        ...
        >
          ...
          <div className="flex-1 overflow-hidden">
            {MOCK_GANTT_TASKS.map((task) => (
              <div
                key={task.id}
                className="h-12 border-b border-[#F1F5F9] px-4 flex items-center gap-3 hover:bg-[#F8FAFC] transition-colors group cursor-pointer"
              >
                <IDChip
                  id={task.id}
                  size="xs"
                  className="font-bold opacity-60 group-hover:opacity-100"
                />
                <span className="text-xs font-bold text-[#475569] truncate group-hover:text-[#0F172A]">
                  {task.name}
                </span>
              </div>
            ))}
            ...
            <div className="relative pt-0">
              {MOCK_GANTT_TASKS.map((task, idx) => {
                ...
                return (
                  <div
                    key={task.id}
                    className="h-12 flex items-center relative border-b border-[#F1F5F9]/30"
                  >
                    <motion.div
                    ...
                    >
                      ...
                      {task.assignee && (
                        <div className="absolute -right-1 -top-1 w-5 h-5 rounded-full border-2 border-white bg-white shadow-sm flex items-center justify-center overflow-hidden scale-0 group-hover:scale-100 transition-transform">
                          <div
                            className={cn(
                              "w-full h-full flex items-center justify-center text-white text-[7px] font-black",
                              task.assignee.color
                            )}
                          >
